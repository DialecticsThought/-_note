...
package com.hmdp.utils.LoginInterceptor 拦截器代码
...
package com.hmdp.config.MvcConfig 让拦截器生效
...
package com.hmdp.service.impl.**UserServiceImpl代码**
...
public Result login(LoginFormDTO loginForm, HttpSession session) {
    ...
    // 7.保存用户信息到 redis中
    // 7.1.随机生成token，作为登录令牌 以前是sessionID
...
package com.hmdp.utils.**RefreshTokenInterceptor**
...
public class RefreshTokenInterceptor implements HandlerInterceptor {
    ...
    * */
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (userMap.isEmpty()) {
            return true;
            //TODO 4 不存在 拦截  (也可以在叠加一个拦截器做这个操作)
            /*response.setStatus(401);
            return false;*/
        }
...
package com.hmdp.utils.**LoginInterceptor**
...
应该具体操作缓存还是操作数据库，**我们应当是先操作数据库，再删除缓存**，原因在于，如果你选择第一种方案，在两个线程并发来访问时，假设线程1先来，他先把缓存删了，此时线程2过来，他查询缓存数据并不存在，此时他写入缓存，当他写入缓存后，线程1再执行更新动作时，实际上写入的就是旧的数据，新的数据被旧数据覆盖了。**(线程1把线程2的操作给冲掉了)**
...
package com.hmdp.service.impl.ShopServiceImpl
...
**package com.hmdp.service.impl.ShopServiceImpl的操作代码：**
...
```java
 public Shop queryWithMutex(Long id)  {
        ...
        //判断命中的值是否是空值  因为防止缓存穿透 会把空值写入redis
        if (shopJson != null) {
            //返回一个错误信息
            return null;
        }
...
public Shop queryWithLogicalExpire( Long id ) {
    ...
    // 4.命中，需要先把json反序列化为对象  把redisData变成shop
...
public class RedisIdWorker {
    ...
    public long nextId(String keyPrefix) {
        ...
        // 2.生成序列号
        // 2.1.获取当前日期，精确到天
        //TODO 2.生成序列号 64位 中地位32bit 利用redis的自增
        //TODO 不能永远使用同一个key 使用拼接时间戳 来代表不同的key 自增的上限 也就代表了一天的量
        String date = now.format(DateTimeFormatter.ofPattern("yyyy:MM:dd"));
        // 2.2.自增长
        long count = stringRedisTemplate.opsForValue().increment("icr:" + keyPrefix + ":" + date);

        // 3.拼接并返回 利用位运算  64位 = 符号位0 + 时间戳31位 + 序列号32bit
...
以上逻辑的核心含义是：只**要我扣减库存时的库存和之前我查询到的库存是一样的，就意味着没有人在中间修改过库存，那么此时就是安全的，但是以上这种方式通过测试发现会有很多失败的情况**，失败的原因在于：在使用乐观锁过程中假设100个线程同时都拿到了100的库存，然后大家一起去进行扣减，但是100个人中只有1个人能扣减成功，其他的人在处理时，他们在扣减时，库存已经被修改过了，所以此时其他线程都会失败
...
**intern() 这个方法是从常量池中拿到数据，如果我们直接使用userId.toString() 他拿到的对象实际上是不同的对象，new出来的对象（每new一次，会返回不同的堆的地址。也就是会生成不同的对象）**，我们使用锁必须保证锁必须是同一把，所以我们需要使用intern()方法
...
Long userId = UserHolder.getUser( ).getId();
synchronized (userId.toString().intern()) {
    //获取代理对象 调用的是当前对象的方法 而不是代理对象 会导致 事务注解失效
...
public  Result createVoucherOrder(Long voucherId) {
	...
	synchronized(userId.toString().intern()){
         ...
         * TODO
         *  利用乐观锁解决超卖问题
         *  stock = seckillVoucher.getStock() 说明 上面的if判断是没有问题的 也说明
         *  在执行 sql代码和上面的if判断之间 其他线程 没有做操作
         *  如果 sql代码和上面的if判断有误差 那么 取消本次操作 并重试
         * */
        // 6.扣减库存
        boolean success = seckillVoucherService.update()
                .setSql("stock = stock - 1") // set stock = stock - 1
...
com.hmdp.utils.**SimpleRedisLock**类
...
* 修改业务代码com.hmdp.service.impl.VoucherOrderServiceImpl
...
```java
  @Override
    public Result seckillVoucher(Long voucherId) {
        ...
        try {
            //获取代理对象(事务) 如果调用的是当前对象的方法 而不是代理对象 会导致 事务注解失效
            IVoucherOrderService proxy = (IVoucherOrderService) AopContext.currentProxy();
            return proxy.createVoucherOrder(voucherId);
        } finally {
            //释放锁
...
```java
    /*
     * TODO 悲观锁 实现 一人一单 防止 多个线程同一时刻判断是否存在该用户订单 会导致判断错误 创建订单
     * */
    @Transactional
    public Result createVoucherOrder(Long voucherId) {
        //TODO 一人一单
...
public Result seckillVoucher(Long voucherId) {
        ...
        Long userId = UserHolder.getUser().getId();
        //TODO 创建锁对象 这个代码不用了，因为我们现在要使用分布式锁
        //SimpleRedisLock lock = new SimpleRedisLock("order:" + userId, stringRedisTemplate);
...
* NOACK：无需手动ACK，获取到消息后自动确认 ，相当于消息不会进入pending-list
* STREAMS key：指定队列名称
* ID：获取消息的起始ID：Eg:  ">"：从下一个未消费的消息开始
  		其它：根据指定id从pending-list中获取已消费但未确认的消息，例如0，是从pending-list中的第一个消息开始（永远给0的话 就可以永远读到pending-list中的第一个消息，直到消息被消费完毕）

消费者监听消息的基本思路：

catch代码块里面处理的是异常 也就是 消息没有被成功消费 进入pending-list

xreadgroup group g1 c1 count 1 stremas s1 0 表示从pending-list的第一个消息消费
...
private class VoucherOrderHandler implements Runnable {
    ...
    public void run() {
        while (true) {
            try {
                // 1.抢购之前 不断地从消息队列获取中订单信息 XREADGROUP GROUP g1 c1 COUNT 1 BLOCK 2000 STREAMS s1 >
                ...
                // 解析数据
                MapRecord<String, Object, Object> record = list.get(0);//TODO 因为count 1 只取一个消息
    ...
    private void handlePendingList() {
        while (true) {
            try {
                // 1.抢购之前 不断地从pendingList取出订单信息 XREADGROUP GROUP g1 c1 COUNT 1 BLOCK 2000 STREAMS s1 0
                List<MapRecord<String, Object, Object>> list = stringRedisTemplate.opsForStream().read(
                    Consumer.from("g1", "c1"),
 ...
 @Override
    public Result follow(Long followUserId, Boolean isFollow) {
        ...
        if (isFollow) {
            ...
            boolean isSuccess = save(follow);//TODO 存到数据库
...
4个参数：
 ...
 最大值（第一次的是当前时间戳，之后每一次都是上一次查询最小时间戳 ） 

最小值（固定不变  ==0） 

偏移量offset 第一次是0 之后 每一次 在上一次结果中 与最小值（时间戳） 一致的元素的个数
...
public Result queryBlogOfFollow(Long max, Integer offset) {
    ...
    long minTime = 0; // 2
    int os = 1; // 2
    // TODO offset 第一次是0   之后 每一次 在上一次结果中 与最小值（时间戳） 一致的元素的个数
    ...
    //TODO 根据id查blog  因为得到的是有序的id 所以也要查到有序的blog
...
void loadShopData() {
    ...
    for (Map.Entry<Long, List<Shop>> entry : map.entrySet()) {
        ...
        // 3.3.写入redis GEOADD key 经度 纬度 member(shopId)
        for (Shop shop : value) {
            // stringRedisTemplate.opsForGeo().add(key, new Point(shop.getX(), shop.getY()), shop.getId().toString());
            locations.add(new RedisGeoCommands.GeoLocation<>(
                    shop.getId().toString(),//TODO 作为member
                    new Point(shop.getX(), shop.getY())//TODO 传入 经纬度
            ));
        }
...
public Result sign() {
    ...
    // 2.获取日期
    LocalDateTime now = LocalDateTime.now();
    // 3.拼接key sign:+id+:202208
    String keySuffix = now.format(DateTimeFormatter.ofPattern(":yyyyMM"));
    String key = USER_SIGN_KEY + userId + keySuffix;
    // 4.获取今天是本月的第几天 一个bitmap 存的是每月的签到数 第几位 就是第几天签到
    int dayOfMonth = now.getDayOfMonth();
    // 5.写入Redis SETBIT key offset 1   1-31 对应计算机 0-30表示
...
public Result signCount() {
    ...
    List<Long> result = stringRedisTemplate.opsForValue().bitField(
            key,
        //TODO 在bitmap中 从角标=0开始 获取 和当前天数 一样的位数
...
布隆过滤器的实现原理是一个超大的**位数组**和**几个哈希函数**。假设位数组的长度为 m，[哈希](https://so.csdn.net/so/search?q=哈希&spm=1001.2101.3001.7020)函数的个数为 k。

![](\Redis实战篇.assets\3803cdaf60db4ef8bcc539d836320683.png)
