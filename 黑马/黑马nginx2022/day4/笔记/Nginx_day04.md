...
所谓四层负载均衡指的是OSI七层模型中的传输层，主要是基于IP+PORT(端口)的负载均衡
...
在反向代理端的nginx做负载均衡器设置

```nginx
# 定义服务组 一个组里面包含了多个服务器的ip+port
upstream backend{
...
```nginx
upstream backend{# 定义服务组 一个组里面包含了多个服务器的ip+port
...
server {
	...
	server_name localhost;
	location /{
		proxy_pass http://backend;# backend就是服务组的名称  由upstream定义
	}
}
```
...
upstream backend{# 定义服务组 一个组里面包含了多个服务器的ip+port
	...
	#当着台服务器出问题之后 再次访问后就会失败 失败次数到达3之后 服务暂停15s
...
当对后端的多台动态应用服务器做负载均衡时，ip_hash指令能够将某个客户端IP的请求通过哈希算法定位到同一台后端服务器上。这样，当来自某一个IP的用户在后端Web服务器A上登录后，在访问该站点的其他URL，能保证其访问的还是后端web服务器A。（当然可以把session放在redis中）
...
##### url_hash(相同的url访问的是相同的web服务器)

按访问url的hash结果来分配请求，使每个url定向到同一个后端服务器，要配合缓存命中来使用。同一个资源多次请求，可能会到达不同的服务器上，导致不必要的多次下载，缓存命中率不高，以及一些资源时间的浪费。而使用url_hash，可以使得同一个url（也就是同一个资源请求）会到达同一台服务器，一旦缓存住了资源，再此收到请求，就可以从缓存中读取。

```nginx
upstream backend{
	hash &request_uri;#内置变量&request_uri用来获取请求的url地址
	server 192.168.200.146:9001;
	server 192.168.200.146:9002;
	server 192.168.200.146:9003;
}
...
客户端有一个请求 来到nginx负载均衡到某一个web服务器 ，该web服务器再向文件系统发请求获取客户所需要的文件。

web收到文件系统的文件后缓存起来，下一次客户再一次需要改文件的话，直接返回文件 而不是访问文件系统
...
4. 使用./configure命令将资源添加到Nginx模块中(需要nginx -V 得到之前的配置)

```
./configure --add-module=/root/fair .....#需要添加之前的配置
```
...
在Nginx的源码中 src/http/ngx_http_upstream.h,找到`ngx_http_upstream_srv_conf_s`，在模块中添加添加default_port属性 （在结构体中的in_port_t	   port下面添加配置）
...
```
cd ../
make upgrade  #在nginx的安装目录地方执行该命令
```
...
```nginx
upstream videobackend{#声明一个服务器组
	server 192.168.200.146:9001;
	server 192.168.200.146:9002;
}
upstream filebackend{#声明一个服务器组
	server 192.168.200.146:9003;
	server 192.168.200.146:9004;
}
server {
	listen 8084;
	server_name localhost;#localhost这里是192.168.200.133
	#访问http://192.168.200.133:8084/video/... 的时候 在videobackend服务器组里面负载均衡
	location /video/ { 
		proxy_pass http://videobackend;
	}
	#访问http://192.168.200.133:8084/file/... 的时候 在filebackend服务器组里面负载均衡
	location /file/ {
...
```nginx
upstream itcastbackend{#声明一个服务器组
	server 192.168.200.146:9001;
	server 192.168.200.146:9002;
}
upstream itheimabackend{#声明一个服务器组
	server 192.168.200.146:9003;
	server 192.168.200.146:9004;
}
server {
	...
	#访问http://www.itcast.cn:8085/  在itcastbackend服务器组里面负载均衡
	location / {
...
server {
	...
	#访问http://www.itheima.cn:8086/ 在itheimabackend服务器组里面负载均衡
	location / {
...
server {
	listen	80;
	server_name localhost;
	#访问http://192.168.200.133:80/file/... 开头的 会重写成 http://192.168.200.133:80/server/...开头的
并且重新匹配location（因为last）  
	location /file/ {
		rewrite ^(/file/.*) /server/$1 last;
	}
	#访问http://192.168.200.133:80/server/... 在backend服务器组里面负载均衡
	location /server {
		proxy_pass http://backend;
	}
}
...
4.使用make和install进行编译和安装( PREFIX指定安装目录)
...
stream {
        ...
        server {
                listen  81;
                proxy_pass redisbackend; #请求分发到 服务器组redisbackend
        }
        server {
        		listen	82;
        		proxy_pass tomcatbackend; #请求分发到 服务器组tomcatbackend
        }
}
```

访问测试。访问192.168.200.133:81 实际上访问的是redis组中的某一个redis

访问192.168.200.133:82实际上访问的是tomcat
...
4.对之前生成的密文截取一部分字母 作为缓存的目录的子目录名 也就是一个缓存目录里面有多个子目录

5.判断缓存目录里面有没有访问的数据对应的目录 有: 从对应的目录获取数据 没有:  从应用服务器拿数据 并且执行上面的操作
...
```nginx
levels=1:2   缓存空间有两层目录，第一次是1个字母，第二次是2个字母   (取字母是从后往前取)
举例说明:
...
该指令用来开启或关闭代理缓存，如果是开启则自定使用哪个缓存区来进行缓存。(要和proxy_cache_path指定的地方一致)
...
```
proxy_cache_valid 200 302 10m; # m代表分钟  响应码是200或302  就缓存10分钟
proxy_cache_valid 404 1m;
为200和302的响应URL设置10分钟缓存，为404的响应URL设置1分钟缓存
proxy_cache_valid any 1m;# 从上往下匹配状态码
...
http{
	...
	server {
        ...
        server_name  localhost;
        location / {
        	proxy_cache itcast; #和keys_zone定义的一样
            proxy_cache_key itheima; #自定义 会被md5加密
            proxy_cache_min_uses 5;#访问统一资源5次以上 才做缓存
            #当访问404时 用itheima作为key 会把内容存在某个文件夹里面 当访问存在的资源时，还是用itheima作为key，回到原先404对应的文件夹里找资源 等到30s之后
            proxy_cache_valid 200 5d; # 响应码200的时候 缓存5天
            proxy_cache_valid 404 30s;# 响应码404的时候 缓存30秒  
            proxy_cache_valid any 1m; # 不管什么响应码 都缓存1分钟 配置越前面 优先级越高
            add_header nginx-cache "$upstream_cache_status";#添加头信息 表示 访问的资源已缓存
        	proxy_pass http://backend/js/;
        }
	}
}
...
指的是当前请求的参数中属性名为nocache和comment对应的属性值 也就是在客户端url后面添加?nocache=xxx&comment=xxx
```

案例演示下:

```
# 定义1种日志格式，取1个名字为params
...
server{
	...
	location /{
		...
		#访问成功之后，会添加一个cookie 也就是<k,v>
		add_header Set-Cookie 'nocache=999';
		#到html的目录里面找 找的是index.html
		root html;
		index index.html;
	}
}
