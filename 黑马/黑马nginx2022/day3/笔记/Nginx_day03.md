...
- **last:终止继续在本location块中处理接收到的URI，并将此处重写的URI作为一个新的URI，再次进入server块,使用各location块进行处理**。该标志将重写后的URI重写在server块中执行，为重写后的URI提供了转入到其他location块的机会。

```nginx
# url路径匹配^/rewrite/(test)\w*$ 这个正则的 会被重写为'/$1'路径 因为last 所以重新location匹配
location rewrite {
...
- redirect：将重写后的URI返回给客户端，状态码为302，**指明是临时重定向URI**,地址栏显示重定向后的url，主要用在replacement变量不是以"http://"或者"https://"开头的情况。
...
server {
        ...
        listen  82;
        resolver 8.8.8.8;#设置DNS的ip 用来解析 proxy_pass中的域名
        location /{
        	   # 客户端->服务端 变成了 客户端->代理->服务端
        	   #代理需要得到客户端要访问的服务端ip和端口和请求路径
...
proxy_pass http://192.168.200.146;
    #访问的是http://192.168.200.146/server/index.html  会把location的 /server拼接上
proxy_pass http://192.168.200.146/;
    #有了"/",访问的是http://192.168.200.146/index.html
```
...
访问测试  http://192.168.200.133:8080/server  实际是http://192.168.200.146:8080/server 
...
server {
    listen  8081;
    server_name localhost;
    if (!-f $request_filename){#如果请求的文件资源不存在的话
    	#重定向  请求资源路径什么都不写的话 就默认到 html目录下的index.html
    	return 302 http://192.168.200.146;
    }
}
```

代理服务端[192.168.200.133] 浏览器访问的是192.168.200.133 实际上是192.168.200.146
...
server {
	...
	location / {
		...
		#第1个参数请求头中的location值  第2个参数是需要替换的
		proxy_redirect http://192.168.200.146 http://192.168.200.133;
	}
}
```

需求：如果客户端通过代理访问服务端但是客户端的请求路径所对应的资源没有的话转发到服务器默认的index.html

**但是这样的话 请求头里面会有location 这个字段标注服务器是哪个地址 这就不能通过反向代理隐藏服务器了☆☆☆☆☆**
...
```
》将原有/usr/local/nginx/sbin/nginx进行备份 备份前./nginx -V把配置粘贴出去 再mv nginx nginxold
》拷贝nginx之前的配置信息
》在nginx的安装源码进行配置指定对应模块 此时 cd /opt/nginx-1.21.1/
 再./configure --with-http_ssl_module和第一步的配置结合再一起
》通过make模板进行编译 因为安装过不需要再 make install
》将objs下面的nginx移动到/usr/local/nginx/sbin下   /opt/nginx-1.21.1/objs/nginx 移动过去
