...
```
kill -TERM master的PID / kill -TERM `cat /usr/local/nginx/logs/nginx.pid`
kill -INT master的PID / kill -INT `cat /usr/local/nginx/logs/nginx.pid`
```
...
4. 反单引号的作用是把反单引号内部命令的结果作为外部命令的输入
...
worker_processes:用于配置Nginx生成工作进程的数量，这个是Nginx服务器实现并发处理服务的关键所在。理论上来说worker process的值越大，可以支持的并发处理量也越多，但事实上这个值的设定是需要受到来自服务器自身的限制，建议将该值和服务器CPU的内核数保存一致。
...
创建一个新的文件 可以和nginx.conf同一个目录 名字叫main.conf

在main.conf文件中写配置，之后在 nginx.conf 中写入include main.conf
...
这里的NAME就是上面access_log的 FORMAT参数
