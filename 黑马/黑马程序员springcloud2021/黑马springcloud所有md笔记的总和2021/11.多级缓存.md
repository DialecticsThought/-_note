...
location  /api/item {
    ...
    # 响应结果由lua/item.lua文件来决定  这个文件最后的结果就是响应
...
--如果 resp得到的是空 则 resp =nil 也就是false的意思
    if not resp then
        -- 记录错误信息，返回404
...
-- 将方法导出 把函数放在一个table里面 名字叫_M 相当于当前的所有东西变成一个整体返回
local _M = {  
    read_http = read_http
}  
return _M
...
```lua
-- 引入自定义common工具模块，返回值是common中返回的 _M common变量是用来接收的 这里不用写路径 因为放在lualib里面
local common = require("common")
-- 从 common中获取read_http这个函数 获取read_http并用一个变量接收
local read_http = common.read_http
-- 获取路径参数
local id = ngx.var[1]
-- 根据id查询商品 而不是之前的假的固定信息
