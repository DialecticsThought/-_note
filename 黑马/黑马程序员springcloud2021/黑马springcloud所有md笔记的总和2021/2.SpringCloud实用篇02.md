...
- [ ] **这个接口 封装了 所有的对userservice所有的远程调用**
- [ ] **这个语法 类似于 springMVC的controller 的语法**
- [ ] **因为是声明式客户端  只要把发http请求所需的信息 全部写出来就可以了**
...
这个配置类是feign模块下的，和feignClient同一个模块
...
- ​      如果xxx是服务名称，例如userservice则代表某服务
  ...
  方式二是 java代码配置Logger.Level这个Bean
...
spring:
  ...
  cloud:
    ...
    gateway:
      routes: # 网关路由配置
        - id: user-service # 路由id：路由的唯一标识，自定义，只要唯一即可
          # uri: http://127.0.0.1:8081 # 路由的目标地址 http就是固定地址 因为 ip地址和端口写死了 不推荐使用
          uri: lb://userservice # 路由的目标地址 lb（loadbalance）就是负载均衡，后面加上服务名称  网关 会根据服务名称 向eureka/nacos拿服务列表 再做负载均衡
          # uri意思就是也就是把某一个http请求 路由到某一个服务
          predicates: # 路由断言，也就是判断请求是否符合路由规则的条件
            - Path=/user/** # 这个是按照路径匹配，只要以/user/开头就符合要求
          filters: # 过滤器
...
在controller层中 Usercontroller 也要作出相应的修改

```
@GetMapping("/{id}")
public User queryById(@PathVariable("id") Long id,
        /* 获取 请求头 这个请求头叫做 Truth */
...
public class AuthorizeFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if ("admin".equals(auth)) {
            // 放行 找到过滤器链的下一个过滤器执行下一个过滤器的filter方法 ==放行
...
不管是路由的过滤器还是defaultFilter 本质都是AddRequestHeaderGatewayFilter， 所以才能合并到一个过滤器集合中

 AddRequestHeaderGatewayFilter工厂 会读取过滤器的配置生成一个真正的过滤器 这个真正过滤器的名字就叫GatewayFilter

有一个GatewayFilterAdapter 它继承了GatewayFilter接口 但是这个类的内部又定义了GlobalFilter  如果传了一个GlobalFilter给这个类的话就进行了适配

只有在请求路由后 才知道有哪些过滤器生效
