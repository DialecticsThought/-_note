...
* beanFactory 需要手动调用方法来初始化单例 因为不会主动初始化单例
...
* postProcessBeforeProperties
  * 这里返回的对象会替换掉原本的 bean
  * 如 @PostConstruct、@ConfigurationProperties
* postProcessAfterProperties
  * 这里返回的对象会替换掉原本的 bean
  * 如代理增强
...
1. 类加载时可以通过 agent 修改 class 字节码实现增强
...
![image-20220330162624771](C:\Users\liujiahao\AppData\Roaming\Typora\typora-user-images\image-20220330162624771.png)
...
* BeanWrapperImpl 为 bean 的属性赋值，当需要时做类型转换，走 Property （因为用反射 所以不能普通赋值） （bean需要有get set方法）
* DirectFieldAccessor 为 bean 的属性赋值，当需要时做类型转换，走 Field（私有成员变量）
...
participant mf as ModelFactory
participant container as ModelAndViewContainer
adapter ->> +bf: 准备 解析控制器上的@InitBinder方法来得到自定义的类型转换器 用来数据绑定和类型转换
bf -->> -adapter: 
adapter ->> +mf: 准备 解析控制器上的@ModelAttribute方法 方法的返回值作为model添加到ModelAndViewContainer
mf ->> +container: 添加Model数据
container -->> -mf: 
mf -->> -adapter: 
```
...
ihm ->> +ar: 1.获取 args
ar ->> ar: 有的解析器涉及 RequestBodyAdvice
ar ->> container: 有的解析器涉及数据绑定生成模型数据并存到Container
container -->> ar: 
ar -->> -ihm: args
ihm ->> ihm: 2.method.invoke(bean,args) 得到 returnValue
ihm ->> +rh: 交给ReturnValueHandler处理 returnValue
rh ->> rh: 有的处理器涉及 ResponseBodyAdvice
rh ->> +container: 添加Model数据,处理视图名,是否渲染等
container -->> -rh: 
rh -->> -ihm: 
ihm -->> -adapter: 
adapter ->> +container: 3.从ReturnValueHandler获取 ModelAndView
container -->> -adapter: 
```
...
1. 在编译时就根据 bean 标注 的 @Indexed 生成 META-INF/spring.components 文件
2. 扫描时
...
1. spring 代理的设计特点
   ...
   * ```
     ...
     ```
     是调用代理对象通过调用get方法间接调用到原始对象的get方法 最终获取的是原始对象所以来的对象
     ```
