    ...
    public static void main(String[] args) throws Throwable {
        ...
        /*
            ...
            其实无论 ProxyFactory 基于哪种方式创建代理, 最后干活(调用 advice)的是一个 MethodInvocation 对象
                ...
                某些通知内部需要调用methodInvocation对象 将 MethodInvocation 放入当前线程
                把methodInvocation方法到当前线程的活也是一个环绕通知ExposeInvocationInterceptor去做
        ...
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        //把除了环绕通知以外的通知变成环绕通知 （代理对象执行方法的时候会会执行的操作）
        ...
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        // 3. 创建并执行调用链 (所有环绕通知 + 目标的信息)
        //某些通知内部需要调用methodInvocation对象 所以要把methodInvocation方法到当前线程
        //把methodInvocation方法到当前线程的活也是一个环绕通知ExposeInvocationInterceptor去做
