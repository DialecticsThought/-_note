...
//        testAnnotationConfigServletWebServerApplicationContext();
    ...
    private static void testAnnotationConfigApplicationContext() {
        //<context:annotation-config/>注解针对beanFactory自动添加一些bean的后置处理器
        //但是如果使用AnnotationConfigApplicationContext 会自动加上
    ...
    static class WebConfig {
        //工厂类提供ServletWebServer  创建一个内嵌的服务器
        @Bean
        ...
        //这个bean是注册DispatcherServlet到服务器
        @Bean
        public DispatcherServletRegistrationBean registrationBean(DispatcherServlet dispatcherServlet) {
            return new DispatcherServletRegistrationBean(dispatcherServlet, "/");
        }
        //上面三个组件可以实现成一个控制器 可以访问 localhost:8080/hello
        @Bean("/hello")
