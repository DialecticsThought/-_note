...
//        test3();
    ...
    private static void test4() throws IOException, HttpMediaTypeNotAcceptableException, NoSuchMethodException {
        ...
        /*
        * 即使请求头制指定了用xml
        * 响应体 指定json 最终还是用json
        * */
        request.addHeader("Accept", "application/xml");
        response.setContentType("application/json");
        /*
        * 这个处理器即可作为参数处理器 可以作为返回值处理器
        * */
        RequestResponseBodyMethodProcessor processor = new RequestResponseBodyMethodProcessor(
                // 默认 json转换器在xml转换器前面 所以json转换器优先  但是请求头/响应体中有指定的话 就是其他情况
                List.of(
        ...
        processor.handleReturnValue(
                new User("张三", 18),
                new MethodParameter(A28.class.getMethod("user"), -1),//方法信息
