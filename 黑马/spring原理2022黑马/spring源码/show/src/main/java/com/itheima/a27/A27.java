...
public class A27 {
    ...
    private static void test7(AnnotationConfigApplicationContext context) throws Exception {
        ...
        HandlerMethodReturnValueHandlerComposite composite = getReturnValueHandler();//得到参数解析器
        MockHttpServletRequest request = new MockHttpServletRequest();//模拟请求
        MockHttpServletResponse response = new MockHttpServletResponse();//模拟响应
        ServletWebRequest webRequest = new ServletWebRequest(request, response);//包装
        /*
         * 这里返回值解析器是RequestResponseBodyMethodProcessor(List.of(new MappingJackson2HttpMessageConverter())
         * 响应体的对象会被MappingJackson2HttpMessageConverter()解析成json格式
         * 解析器有一个方法handleReturnValue中mavHandler.setRequestHandled（true） 那么 就不会走视图的流程
         * */
    ...
    private static void test6(AnnotationConfigApplicationContext context) throws Exception {
        ...
        * */
        ...
        ServletWebRequest webRequest = new ServletWebRequest(request, response);//包装
        /*
         * 这里返回值解析器是 HttpHeadersReturnValueHandler()
         * 解析器有一个方法handleReturnValue中mavHandler.setRequestHandled（true） 那么 就不会走视图的流程
         * */
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            composite.handleReturnValue(returnValue, methodHandle.getReturnType(), container, webRequest);
    ...
    private static void test5(AnnotationConfigApplicationContext context) throws Exception {
        ...
        ServletWebRequest webRequest = new ServletWebRequest(request, response);//包装
        /*
        * 这里返回值解析器是HttpEntityMethodProcessor(List.of(new MappingJackson2HttpMessageConverter())
        * 响应体的对象会被MappingJackson2HttpMessageConverter()解析成json格式
         * 解析器有一个方法handleReturnValue中mavHandler.setRequestHandled（true） 那么 就不会走视图的流程
        * */
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
    ...
    private static void test4(AnnotationConfigApplicationContext context) throws Exception {
        ...
        HandlerMethodReturnValueHandlerComposite composite = getReturnValueHandler();//得到参数解析器
        MockHttpServletRequest request = new MockHttpServletRequest();//模拟请求
        request.setRequestURI("/test4");//设置了以后 会找名字为test4的视图
        UrlPathHelper.defaultInstance.resolveAndCacheLookupPath(request);
        ServletWebRequest webRequest = new ServletWebRequest(request, new MockHttpServletResponse());
        //这里返回值解析器是ServletModelAttributeMethodProcessor（false）
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            composite.handleReturnValue(returnValue, methodHandle.getReturnType(), container, webRequest);
            System.out.println(container.getModel()); //得到模型
            System.out.println(container.getViewName());//得到视图名称 根据视图名称获得模板页面
            renderView(context, container, webRequest); // 渲染视图
        }
    }
    ...
    private static void test3(AnnotationConfigApplicationContext context) throws Exception {
        /*
        * 因为该方法有 @ModelAttribute
        * 那么方法的返回值会作为模型数据添加到ModelContainer里面
        * 因为方法有@RequestMapping("/test3") /test3作为视图名称
        * */
        ...
        HandlerMethodReturnValueHandlerComposite composite = getReturnValueHandler();//得到参数解析器
        MockHttpServletRequest request = new MockHttpServletRequest();//模拟请求
        request.setRequestURI("/test3");//设置了以后 会找名字为test3的视图
        UrlPathHelper.defaultInstance.resolveAndCacheLookupPath(request);
        ServletWebRequest webRequest = new ServletWebRequest(request, new MockHttpServletResponse());
        //这里返回值解析器是ServletModelAttributeMethodProcessor
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            composite.handleReturnValue(returnValue, methodHandle.getReturnType(), container, webRequest);
            System.out.println(container.getModel());//得到模型
            System.out.println(container.getViewName());//得到视图名称 根据视图名称获得模板页面
    ...
    private static void test2(AnnotationConfigApplicationContext context) throws Exception {
        ...
        Controller controller = new Controller();
        Object returnValue = method.invoke(controller); // 获取返回值“view2”
        ...
        //这里返回值解析器用到了ViewNameMethodReturnValueHandler() 因为返回值是视图名称
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            //返回值的对象 方法信息  ModelContainer（如果返回值是模型数据的话 用到）  请求
            composite.handleReturnValue(returnValue, methodHandle.getReturnType(), container, webRequest);
            System.out.println(container.getModel());//得到模型
            System.out.println(container.getViewName());//得到视图名称 根据视图名称获得模板页面
            renderView(context, container, webRequest); // 渲染视图
    ...
    private static void test1(AnnotationConfigApplicationContext context) throws Exception {
        ...
        Controller controller = new Controller();
        //方法调用
        Object returnValue = method.invoke(controller); // 获取方法返回值
        ...
        HandlerMethodReturnValueHandlerComposite composite = getReturnValueHandler();
        ServletWebRequest webRequest = new ServletWebRequest(new MockHttpServletRequest(), new MockHttpServletResponse());
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查返回值处理器是否支持此类型的返回值
            //返回值的对象 方法信息  ModelContainer（如果返回值是模型数据的话 用到）  请求
            composite.handleReturnValue(returnValue, methodHandle.getReturnType(), container, webRequest);
            System.out.println(container.getModel());//得到模型
            System.out.println(container.getViewName());//得到视图名称 获得模板页面
            renderView(context, container, webRequest); // 渲染视图： 把模型数据渲染到模板页面
        }
    }
