...
![](H:\笔记\尚硅谷SSM项目（硅谷通用权限项目）2022\笔记\images\15-角色管理前端内容概述.png)
...
![](H:\笔记\尚硅谷SSM项目（硅谷通用权限项目）2022\笔记\images\16-搭建前端环境.png)
...
![](H:\笔记\尚硅谷SSM项目（硅谷通用权限项目）2022\笔记\images\18-角色列表实现步骤.png)
...
save(role) {
  return request({
    url: `${api_name}/save`,
    method: 'post',
      // 在请求体传递 json格式的数据  和后端@ReuqestBody对应
    data: role
  })
}
```
...
// 点击  添加或更新 按钮
saveOrUpdate() {
  if (!this.sysRole.id) {//如果存在id 就是更新
    this.save()
  } else {
    this.update()
  }
...
handleSelectionChange(selection) {
  ...
  this.multipleSelection = selection //存入 选择的复选框id
},
// 批量删除
batchRemove() {
  ...
  }).then((response) => { // 后端响应成功的操作
     //重新获取 数据
    this.fetchData()
    this.$message.success(response.message)
  }).catch(error => { // 后端响应失败的操作
    if (error === 'cancel') {
      this.$message.info('取消删除')
    }
  })
